<template>
  <div class="components PhotosAlbumMove">
    <Popup theme="black" :show="true" :ShowBottom="ShowPopupBottomBtn" type="confirm" :title="SubTitle === null ? Title : SubTitle" @Close="$emit('Close')" @Sure="MoveFolderPopupSure">
        ...
        <div class="flex-item">
          <div>
            <ScrollY Size="small">
              <!-- 图集列表 -->
              <ul>
                <li v-for="(item, index) in PhotoAlbum" :key="index" @click="CutMoveFolder(item, index)" class="flex-h" :class="{'active': SelectFolderIndex === index}">
                  <a class="iconfont" :class="[SelectFolderIndex === index ? 'iconyitianjia' : 'iconyuan']"></a>
                  <span class="flex-item">{{item.title}}</span>
                  <span>{{item.defaults - 0 === 1 ? item.photoNum : `${item.photoNum}/255`}}</span>
                </li>
              </ul>
            </ScrollY>
          </div>
      ...
      <!-- <div class="AddNew" v-else>
        ...
        <div class="Title">
          <span>Gallery Introduction </span>
    ...
    <Popup v-if="false" theme="black" :show="true" type="confirm" :title="Title" @Close="$emit('Close')" @Sure="MoveFolderPopupSure">
      <div class="MoveFolderPopupContent">
        ...
        <!-- 新建收藏夹 -->
        <div class="AddFolder flex-h" v-if="PhotoAlbum.length < 11">
...
export default {
  ...
  created () {
    if (!this.PhotoAlbum.length) {
      this.GetPhotoAlbum({ params: { accountId: this.UserInfo.id } })
    }
  },
  mounted () {
    this.$nextTick(() => {
  ...
  methods: {
    ...
    ShowAddFolder () { // 显示新建弹窗
      // this.SubTitle = 'Gallery Information' // 弹窗标题：默认显示props中的Title
      ...
      if (!this.DataLock && this.SelectFolderIndex !== null) {
        ...
        this.AddVideoCollect({
          params: {
            ...
            categoryId: this.PhotoAlbum[this.SelectFolderIndex].id
          }
        }).then(() => {
          this.DataLock = false
          this.$notify.success({
          ...
          this.GetPhotoAlbum({ params: { accountId: this.UserInfo.id } }).then(() => {
            this.$emit('Close')
          })
          this.$emit('Success')
        }).catch((res) => {
