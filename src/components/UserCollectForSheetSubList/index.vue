<template>
  <div class="components UserCollectForSheetSubList flex-v">

    <!-- 封面 -->
    <div class="Front flex-h">
      <div class="Img">
        <a>
          <img v-if="FavoritesIndex !== null" :src="CollectForSheetList[FavoritesIndex].cover" class="centerLabel contain" alt="">
        </a>
      </div>
      <div class="flex-item flex-v">
        <span class="flex-item">{{FavoritesIndex !== null ? CollectForSheetList[FavoritesIndex].name : ''}}</span>
        <div>
...
export default {
  name: 'UserCollectForSheetSubList',
  props: ['data'],
  data () {
    return {
  ...
  computed: {
    ...mapUserState({
      ...
      MyPieceGroupSubList: x => x.MyPieceGroupSubList, // 收藏夹下视频列表
  ...
  methods: {
    ...
    LinkToPlayer (item) {
      window.open(`./video.html?key=${item.videoM}&type=4&sheet=${this.CollectForSheetList[this.FavoritesIndex].id}`, '_self')
    },
    ...
    ToGetVideoCollect () { // 获取收藏夹下视频列表
      if (!this.DataLock) {
        ...
        this.GetPieceGroupSubList({ params: { groupId: this.ToolClass.GetUrlParams('sheet'), ...this.PageData } }).then((res) => {
