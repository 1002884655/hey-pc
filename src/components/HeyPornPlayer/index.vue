<template>
  <div class="components HeyPornPlayer" :class="{'fullscreen': IsFullscreen}">
...
export default {
  name: 'HeyPornPlayer',
  props: ['options', 'srcList', 'activeIndex', 'IsFullWindow', 'FrontPath', 'IsIframe', 'CurrentTimeTamp', 'OnFocus', 'IsFree'],
  data () {
    return {
  ...
  methods: {
    Cut () {
      this.NormalActiveIndex = this.activeIndex - 0 || 0
      let BlackHoleVideo = videojs('BlackHoleVideo') // 获取videojs实例
      BlackHoleVideo.src([{ type: 'application/x-mpegURL', src: this.srcList[this.NormalActiveIndex].src }])
    },
    Init () { // 初始化
      let _that = this
      videojs('BlackHoleVideo', { ...this.VideoOptions }, function () { // 初始化播放器回调（加入自定义控件）
        ...
        VideoHotKeyList.innerHTML = `
          <li>
            <div><img src="${require('../../assets/img/keyboard.png')}" style="left: 0px" /></div>
            <span>Mute / Unmute</span>
          </li>
          <li>
            <div style="width: 84px"><img src="${require('../../assets/img/keyboard.png')}" style="left: -40px" /></div>
            <span>Play / Pause</span>
          </li>
          <li>
            <div><img src="${require('../../assets/img/keyboard.png')}" style="left: -124px" /></div>
            <span>Toggles Fullscreen</span>
          </li>
          <li>
            <div><img src="${require('../../assets/img/keyboard.png')}" style="left: -164px" /></div>
            <div><img src="${require('../../assets/img/keyboard.png')}" style="left: -204px" /></div>
            <span>Volume up / down</span>
          </li>
          <li>
            <div><img src="${require('../../assets/img/keyboard.png')}" style="left: -244px" /></div>
            <span>Slow motion on / off</span>
          </li>
          <li>
            <div><img src="${require('../../assets/img/keyboard.png')}" style="left: -284px" /></div>
            <div><img src="${require('../../assets/img/keyboard.png')}" style="left: -324px" /></div>
        ...
        document.getElementsByClassName('CopyUrl')[0].onclick = () => { // 复制视频链接
          _that.$copyText(`${window.location.origin}/p/playeriframe.html?key=${_that.ToolClass.GetUrlParams('key')}`)
        }

        document.getElementsByClassName('CopyUrlByTime')[0].onclick = () => { // 复制视频链接（附带当前时间节点）
          _that.$copyText(`${window.location.origin}/p/playeriframe.html?key=${_that.ToolClass.GetUrlParams('key')}&time=${Math.floor(this.currentTime())}`)
        }

        document.getElementsByClassName('CopyEmbedCode')[0].onclick = () => { // 复制视频嵌入链接
          _that.$copyText(`<iframe width="1903" height="800" src="${window.location.origin}/p/playeriframe.html?key=${_that.ToolClass.GetUrlParams('key')}" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>`)
        }
        ...
        DefaultWindow.className = `DefaultWindow iconfont iconguanbi`
        ...
        MiniDrag.onmousedown = (e) => {
          ...
          MiniDrag.onmousemove = (e) => {
            e = e || event
            _that.$emit('PositionChange', [e.pageX - MiniStartX, e.pageY - MiniStartY])
          }
          MiniDrag.onmouseup = () => {
            MiniDrag.onmousemove = null
            _that.$emit('PositionChangeEnd')
          }
          MiniDrag.onmouseout = () => {
            MiniDrag.onmousemove = null
            _that.$emit('PositionChangeEnd')
          }
          MiniDrag.onmouseleave = () => {
            MiniDrag.onmousemove = null
            _that.$emit('PositionChangeEnd')
          }
        }
        ...
        for (let n = 0; n < _that.srcList.length; n++) {
          ControlDefinitionHtml += `<li class="${n === _that.activeIndex ? 'active' : ''}"><i class="iconfont icongou"></i>${_that.srcList[n].value}</li>`
        }
        ...
        ControlDefinition.onclick = (e) => {
          e = e || event
          e.stopPropagation()
          ControlDefinitionList.style.display = 'block'
          ControlSettingList.style.display = 'none'
          ControlVoiceSlider.style.display = 'none'
        }
        ...
        ControlSetting.onclick = (e) => {
          e = e || event
          e.stopPropagation()
          ControlSettingList.style.display = 'block'
          ControlDefinitionList.style.display = 'none'
          ControlVoiceSlider.style.display = 'none'
        }
        ...
        ControlSpeedList.style.top = -(_that.VideoOptions.playbackRates.length - 1) * 40 - 30 + 'px'
        let ControlSpeedHtml = ''
        for (let n = 0; n < _that.VideoOptions.playbackRates.length; n++) {
          ControlSpeedHtml += `<li class="${_that.VideoOptions.playbackRates[n] - 0 === 1 ? 'active' : ''}"><i class="iconfont icongou"></i>${_that.VideoOptions.playbackRates[n]}x</li>`
        }
        ...
        for (let n = 0; n < ControlSpeedList.children.length; n++) {
          ControlSpeedList.children[n].onclick = _that.CutSpeed(ControlSpeedList, n, ControlSettingList)
          _that.SpeedIndex = n
        }
        ...
        ControlVoice.onclick = (e) => {
          e = e || event
          e.stopPropagation()
          ControlVoiceSlider.style.display = 'block'
          ControlSettingList.style.display = 'none'
          ControlDefinitionList.style.display = 'none'
        }
        _that.ToolClass.WindowClick(() => {
          ControlSettingList.style.display = 'none'
          ControlDefinitionList.style.display = 'none'
          ControlVoiceSlider.style.display = 'none'
        })
        ...
        if (document.pictureInPictureEnabled) {
          ...
          // 创建画中画tips
          let ControlPIPBtnTips = document.createElement('span')
          ControlPIPBtnTips.className = 'ControlPIPBtnTips'
          ControlPIPBtnTips.innerHTML = 'Miniplayer'
          ControlPIPBtn.appendChild(ControlPIPBtnTips)
        }
        ...
        if (!_that.IsIframe) {
          ...
          // 创建剧场模式按钮tips
          let ControlFullWindowTips = document.createElement('span')
          ControlFullWindowTips.className = 'ControlFullWindowTips'
          ControlFullWindowTips.innerHTML = 'FullWindow'
          ControlFullWindow.appendChild(ControlFullWindowTips)
        }
        ...
        // 创建全屏模式按钮tips
        let ControlFullScreenTips = document.createElement('span')
        ControlFullScreenTips.className = 'ControlFullScreenTips'
        ControlFullScreenTips.innerHTML = 'FullScreen'
        ControlFullScreen.appendChild(ControlFullScreenTips)
        ...
        ProgressThumb.innerHTML = '<div><img class="ThumbSrc cover" src="" /><span class="ThumbTime">00:00:00</span></div>'
        if (!_that.IsFree) {
        ...
        document.onkeydown = (e) => {
          ...
          if (e.keyCode === 83 && _that.OnFocus) { // S键（快慢切换）
            ...
            _that.CutSpeed(ControlSpeedList, _that.SpeedIndex, ControlSettingList)()
            // return false
          }
        }
    ...
    TriggerFullScreen (target) { // 切换全屏
      return () => {
        let BlackHoleVideo = videojs('BlackHoleVideo') // 获取videojs实例
        // console.log(BlackHoleVideo.muted())
    ...
    CutDefinition (target, index) { // 切换清晰度
      return () => {
        ...
        this.CutCurrentTime = BlackHoleVideo.currentTime() - 0
        BlackHoleVideo.src([{ type: 'application/x-mpegURL', src: this.srcList[index].src }])
        window.setTimeout(() => {
          BlackHoleVideo.currentTime(this.CutCurrentTime)
          this.PlayTrigger()
    ...
    CutSpeed (target, index, pTarget) { // 切换倍速
      return () => {
        let BlackHoleVideo = videojs('BlackHoleVideo') // 获取videojs实例
      ...
      this.VideoTimer = window.setInterval(() => {
        if (BlackHoleVideo.duration()) {
          ...
          this.PlayTimer = window.setInterval(() => { // 监听播放进度
            ...
            ControlTime.innerText = (this.ToolClass.DateFormat(BlackHoleVideo.currentTime() * 1000) || '00:00') + '/' + this.ToolClass.DateFormat(BlackHoleVideo.duration() * 1000) // 计算显示播放总时长
          }, 100)
          ...
          target.onmousemove = (e) => { // 鼠标滑动显示缩略图
            ...
            let CutTime = Math.floor(BlackHoleVideo.duration() * CutPercent)
            let CutImgIndex = Math.floor(CutTime / 25) + 1
            let CutImgItemIndex = CutTime % 25 + 1
            ...
            document.getElementsByClassName('ThumbSrc')[0].src = `${this.FrontPath}Image3/merged${CutImgIndex}.jpg`
            document.getElementsByClassName('ThumbSrc')[0].style.top = `-${Math.floor((CutImgItemIndex - 1) / 5) * 90}px`
            document.getElementsByClassName('ThumbSrc')[0].style.left = `-${((CutImgItemIndex % 5 || 5) - 1) * 160}px`
          }
        }
      }, 13)
    },
