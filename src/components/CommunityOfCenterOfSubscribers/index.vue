<template>
  <div class="components CommunityOfCenterOfSubscribers">
    ...
    <ul>
      <li v-for="(item, index) in SubscribeForMeList" :key="index" class="flex-h">
        ...
        <a :class="{'active': !item.subscribeStatus}" @click="TriggerItem(item, index)">{{!item.subscribeStatus ? 'Subscribers' : 'Subscribed'}}</a>
        <!-- <div class="flex-h">
          <div class="Icon">
            <a :href="`./userspace.html?key=${item.userId}`" target="_self">
              <img :src="item.picPath2" class="centerLabel" alt="">
...
export default {
  name: 'CommunityOfCenterOfSubscribers',
  props: ['data'],
  data () {
    return {
      Total: 0,
  ...
  computed: {
    ...mapUserState({
      SubscribeForMeList: x => x.SubscribeForMeList // 订阅该用户的列表
    })
  },
  components: {
  ...
  methods: {
    ...
    TriggerItem (item, index) { // 订阅、取消订阅
      if (!this.DataLock) {
        this.DataLock = true
        if (!item.subscribeStatus) { // 订阅
          this.AccountSubscribe({
            params: { subId: item.userId, accountId: this.data.id }
          }).then(() => {
            this.EditSubscribeForMeList({ Index: index, Active: 1 })
            this.DataLock = false
          }).catch((res) => {
          ...
          this.Unsubscribe({
            ...
            this.EditSubscribeForMeList({ Index: index, Active: 0 })
            this.DataLock = false
          }).catch((res) => {
            this.$notify.error({ title: 'error', message: res.data.msg })
            this.DataLock = false
