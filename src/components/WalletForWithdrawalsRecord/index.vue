<template>
  <div class="components WalletForWithdrawalsRecord">
    <div class="Filter">
      <el-select class="Select" v-model="WithdrawalsRecordDate" placeholder="Select bill date" no-data-text="no data" @change="TypeChange">
        <el-option v-for="(item, index) in WithdrawalsRecordDateList" :key="index" :label="item" :value="item"></el-option>
      </el-select>
    </div>
    <div class="List">
      ...
      <div class="Line flex-h" v-for="(item, index) in PageList" :key="index">
        ...
        <span class="flex-item">{{ToolClass.ReturnBillStatus(item.status) === 'processing' ? 'reviewing' : ToolClass.ReturnBillStatus(item.status) === 'successful' ? 'successful' : 'failed'}}</span>
        <span class="flex-item"><a @click="$emit('ShowDetail', { Data: item, PopupType: 'withdraw' })">Details</a></span>
      </div>
    </div>
...
export default {
  name: 'WalletForWithdrawalsRecord',
  props: ['data'],
  data () {
    return {
      Total: 0,
      ...
      PageList: [],
      WithdrawalsRecordDate: null,
      WithdrawalsRecordDateList: []
    }
  },
  computed: {
    ...mapUserState({
  ...
  methods: {
    ...
    DataListInit () { // 日期初始化
      ...
      for (let n = Month; n > 0; n--) {
        this.WithdrawalsRecordDateList.push(`${Year}-${n > 9 ? n : `0${n}`}`)
      }
      if (Month < 12) {
        for (let n = 0; n < 12 - Month; n++) {
          this.WithdrawalsRecordDateList.push(`${Year - 1}-${12 - n > 9 ? 12 - n : `0${12 - n}`}`)
        }
      }
      this.WithdrawalsRecordDate = this.WithdrawalsRecordDateList[0]
      this.ToGetPageList()
    },
    ToGetPageList () { //  获取列表
      this.GetWithdrawList({
        data: {
          ...
          dateTime: this.WithdrawalsRecordDate,
          ...this.PageData
        }
      }).then((res) => {
