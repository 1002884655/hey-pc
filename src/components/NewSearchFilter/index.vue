<template>
  <div class="components NewSearchFilter" :class="{'active': ShowFilter}">

    <a class="TriggerIcon" @click="ShowFilter = !ShowFilter">
      <i class="iconfont iconsuohui3-2"></i>
      <span class="centerLabel iconfont" :class="[ShowFilter ? 'iconjiantouleft' : 'iconjiantouright']"></span>
    </a>
    ...
    <ScrollY Size="small">
      ...
      <ul class="NavLevel3List" v-show="ShowNavLevel3Filter">
        <li v-for="(item, index) in NavLevel3List" :key="index" v-if="item.Name === 'Category'">
          <span @click="item.IsOpen = !item.IsOpen">{{item.Name}} <i class="iconfont iconleimupinleifenleileibie2"></i></span>
          <div class="flex-item" :class="{'active': item.IsOpen}">
        ...
        if (res.data.code - 0 === 0) {
          ...
          for (let key in res.data.data) { // 构建过滤器数据
            if (key !== 'orderByList' && key !== 'areaList') { // 三级过滤器数据构建
              ...
              this.$emit('SortReady', this.NavLevel2List)
            }
          }
