<template>
  <div class="components UserForMySheetVideosCopy">
        ...
        <div class="flex-item">
          <div>
            <ScrollY Size="small">
              <!-- 片单列表 -->
              <ul>
                <li v-for="(item, index) in MyPieceGroupList" :key="index" @click="CutMoveFolder(item, index)" class="flex-h" :class="{'active': SelectFolderIndex === index}">
...
export default {
  ...
  created () {
    if (this.MyPieceGroupList.length) {
      ...
      this.GetPieceGroup({ params: { accountId: this.UserInfo.id, pageNum: 1, pageSize: 255 } })
    }
  },
  mounted () {
    this.$nextTick(() => {
  ...
  methods: {
    ...
    AddCollect () { // 添加到片单
      if (!this.DataLock && this.SelectFolderIndex !== null) {
        ...
        this.AddPieceGroupSubItem({
          params: {
            videoIds: this.CollectId,
            accountId: this.UserInfo.id,
            groupIds: this.MyPieceGroupList[this.SelectFolderIndex].id
          }
        }).then(() => {
          this.DataLock = false
          this.$notify.success({
    ...
    AddFolder () { // 新建片单
      if (!this.DataLock && this.NewFolderName !== '') {
        ...
        this.AddPieceGroup({ data: { name: this.NewFolderName, accountId: this.UserInfo.id, description: this.NewFolderDesc } }).then(() => {
          this.NewFolderName = ''
