<template>
  <div class="components PlayerComment">
    <div class="Title flex-h" id="MediaCommentTitle">
      <span>Comment</span>
    ...
    <div class="CommentTopic">
      <textarea cols="30" rows="2" placeholder="Add a comment…" v-model="MainCommentValue" @input="MainCommentInput" class="TopicInput" @focus="MainCommentFocus" @blur="MainCommentInputIndex = GetCursortPosition(`TopicInput`)"></textarea>
      <div class="flex-h">
        <div class="flex-item">
          <div>
      ...
      <li class="flex-h" v-for="(item, index) in CommentList" :key="index">
        ...
        <div class="flex-item">
          <div>
            ...
            <!-- 主评论人用户名及评论时间 -->
            <div class="Name">
              <span>{{item.UserName === 'self' ? UserInfo.nick : item.UserName}}</span>
              <span>{{item.CreateDate | changeTime}}</span>
            </div>

            <!-- 主评论内容 -->
            <div class="Text" :class="{'active': item.ShowAllText}">{{item.Text}}</div>

            <div class="ShowMoreText" v-if="item.Text.length > 100"><a @click="TriggerAllMainCommentText(index)">{{item.ShowAllText ? 'Read Less' : 'Read More'}}</a></div>
            ...
            <div class="Operation">
              ...
              <a class="ReplyBtn" @click="TriggerReplyInput(index)">{{ShowReplyIndex === index ? 'Collapse' : 'Reply'}}</a>
            </div>

            <!-- 一级评论回复输入框 -->
            <div class="ReplyInput" :class="{'active': ShowReplyIndex === index}">
              <!-- <div style="width: 30px;position: relative;margin: 0">
                <a class="iconfont iconbiaoqing" :class="`Level1Reply${index}`" style="font-size: 20px;bottom: 5px;position:absolute;left: 50%;transform: translateX(-50%);-webkit-transform: translateX(-50%);" @click="ShowEmoji(`Level1Reply${index}`, 'SubCommentContent')"></a>
              </div> -->
              <div class="flex-item flex-v">
                <div class="flex-item">
                  <textarea cols="30" rows="2" placeholder="Add a comment…" v-model="SubCommentContent" @input="SubCommentInput" :class="`SubInput${index}`" @focus="ShowEmojiBoxer = false" @blur="SubCommentInputIndex = GetCursortPosition(`SubInput${index}`)"></textarea>
                </div>
                <div>
                  <a class="iconfont iconbiaoqing" :class="`Level1Reply${index}`" @click="ShowEmoji(`Level1Reply${index}`, 'SubCommentContent')"></a>
                </div>
              </div>
              <a @click="SubmitReply(index, null, null, item.Id)" :class="{'active': SubCommentContent.length}"><span class="centerLabel">Reply</span></a>
            </div>

            <!-- 显隐主评论回复列表按钮 -->
            <a class="red" v-if="item.ReplyNum - 0" @click="TriggerReplyList(item, index)" :class="{'active': item.ShowReplyList}">
              <i v-if="!item.ShowReplyList" class="iconfont iconshixinjiantoudown"></i>
              <i v-else class="iconfont iconshixinjiantouup"></i>
              <span>{{item.ShowReplyList ? item.ReplyNum - 0 > 1 ? `Hide ${item.ReplyNum} Replies` : 'Hide Reply' : item.ReplyNum - 0 > 1 ? `View ${item.ReplyNum} Replies` : 'View Reply'}}</span>
            </a>
            ...
            <ul class="ReplyList" :class="{'active': item.ShowReplyList}">
              <li class="flex-h" v-for="(subItem, subIndex) in item.List" :key="subIndex">
                <div class="UserIcon">
                  <a>
                    <img :src="subItem.UserIcon === 'self' ? UserInfo.picPath1 : subItem.UserIcon" class="centerLabel cover" alt="">
                  </a>
                </div>
                <div class="flex-item">
                  <div>
                    ...
                    <div class="Name">
                      ...
                      <span>{{subItem.CreateDate | changeTime}}</span>
                    </div>

                    <!-- 二级评论内容 -->
                    <div class="Text" :class="{'active': subItem.ShowAllText}"><em v-if="subItem.ReplyName">@{{subItem.ReplyName}}</em>{{subItem.Text}}</div>

                    <div class="ShowMoreText" v-if="subItem.Text.length > 100"><a @click="TriggerAllSubCommentText(index, subIndex)">{{subItem.ShowAllText ? 'Read Less' : 'Read More'}}</a></div>
                    ...
                    <div class="Operation">
                      ...
                      <a class="ReplyBtn" @click="TriggerSubReplyInput(index, subIndex, subItem.UserName, subItem.AccountId)">{{ShowSubReplyIndex[0] === index && ShowSubReplyIndex[1] === subIndex ? 'Collapse' : 'Reply'}}</a>
                    </div>

                    <!-- 二级评论回复输入框 -->
                    <div class="ReplyInput" :class="{'active': ShowSubReplyIndex[0] === index && ShowSubReplyIndex[1] === subIndex}">
                      <!-- <div style="width: 30px;position: relative;margin: 0">
                        <a class="iconfont iconbiaoqing" :class="`Level2Reply${index}-${subIndex}`" style="font-size: 20px;bottom: 5px;position:absolute;left: 50%;transform: translateX(-50%);-webkit-transform: translateX(-50%);" @click="ShowEmoji(`Level2Reply${index}-${subIndex}`, 'SubCommentContent')"></a>
                      </div> -->
                      <div class="flex-item flex-v">
                        <div class="flex-item">
                          <textarea cols="30" rows="2" placeholder="Add a comment…" v-model="SubCommentContent" @input="SubCommentInput" :class="`SubInput${index}-${subIndex}`" @focus="ShowEmojiBoxer = false" @blur="SubCommentInputIndex = GetCursortPosition(`SubInput${index}-${subIndex}`)"></textarea>
                        </div>
                        <div>
                          <a class="iconfont iconbiaoqing" :class="`Level1Reply${index}`" @click="ShowEmoji(`Level1Reply${index}`, 'SubCommentContent')"></a>
                        </div>
                      </div>
                      <a @click="SubmitReply(index, SubAtId, SubAtName, item.Id)" :class="{'active': SubCommentContent.length}"><span class="centerLabel">Reply</span></a>
                    </div>
                  </div>
                </div>
              </li>
...
export default {
  ...
  methods: {
    ...
    ...mapMediaMutations([
      ...
      'AddSubComment',
      'EmptyCommentList',
      'TriggerMainCommentText',
      'TriggerSubCommentText'
    ]),
    MainCommentInput () { // 主评论限制500字符输入
      this.MainCommentValue = this.MainCommentValue.substring(0, 500)
    },
    SubCommentInput () { // 12级评论限制500字符输入
      this.SubCommentContent = this.SubCommentContent.substring(0, 500)
    },
    TriggerAllMainCommentText (index) { // 显隐全部主评论内容
      this.TriggerMainCommentText(index)
    },
    TriggerAllSubCommentText (index, subIndex) { // 显隐全部2级评论内容
      this.TriggerSubCommentText([index, subIndex])
    },
    ...
    SubmitReply (index, atId, atName, id) { // 一二级评论回复提交
      if (this.SubCommentContent.length) {
